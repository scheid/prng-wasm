...
# It seems -O3 optimization may accidentally slice _malloc() from generated code, so we use 02 here
  /home/eischeid/web-assembly/emsdk/fastcomp/emscripten/emcc.py -o prng-wasm.js mt19937-64.c prng-utils-mt19937.c md5.c -O1 -s WASM=1 -s EXPORT_NAME='PrngEnscriptenModule' -s NO_EXIT_RUNTIME=1 -s MALLOC='dlmalloc' -s FILESYSTEM=0 -s MODULARIZE=1 -s EMIT_EMSCRIPTEN_METADATA=1 -s EXPORT_ES6=1 -s ENVIRONMENT='web' -s FILESYSTEM=0 -s EXPORTED_FUNCTIONS="['_main', '_malloc']"  -s "EXTRA_EXPORTED_RUNTIME_METHODS=['ccall']"
